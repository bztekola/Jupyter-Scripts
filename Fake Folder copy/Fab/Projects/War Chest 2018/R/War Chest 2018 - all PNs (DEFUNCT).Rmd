...
### Check out the unique part id count
```{r}
...
### Put each part through each model, choose the best model based on the RMSE
```{r}
start <- Sys.time()
...
### Check to make sure you made a model for each PN
```{r}
...
### Check out avg MAPE and MAE of all the models
```{r}
# round these numbers
...
parts_main <- rbind(used,excluded)
...
### Tabs 02 through 06: Part forecasts segmented by popularity tier
```{r}
